...
impl PascalParser {
    ...
    pub(super) fn parse_declaration_part(&mut self) -> Result<DeclarationPart, SyntaxError> {
        ...
        // TODO (CST): Logika loop ini sebagian besar sama dengan AST.
        // 1. Loop `while self.check_keyword("konstanta")`:
        //    panggil `self.parse_constant_declaration()` dan push hasilnya.
        // 2. Loop `while self.check_keyword("tipe")`:
        //    panggil `self.parse_type_declaration()` dan push hasilnya.
        // 3. Loop `while self.check_keyword("variabel")`:
        //    panggil `self.parse_variable_declaration()` dan push hasilnya.
        // 4. Loop `while self.check_keyword("prosedur") || self.check_keyword("fungsi")`:
        //    panggil `self.parse_subprogram_declaration()` dan push hasilnya.
    ...
    fn parse_constant_declaration(&mut self) -> Result<ConstantDeclaration, SyntaxError> {
        ...
        //    (Pastikan loop berhenti jika tidak ada lagi Identifier, 
        //     memungkinkan blok `konstanta` kosong jika diikuti `tipe` dll.)
    ...
    fn parse_subprogram_declaration(&mut self) -> Result<SubprogramDeclaration, SyntaxError> {
        // TODO (CST): Logika ini seharusnya sama persis dengan AST.
    ...
    pub(super) fn parse_type_spec(&mut self) -> Result<Type, SyntaxError> {
        ...
        //    - Panggil `self.parse_array_type()` (dia akan meng-consume 'larik' dan return Ok(Type::Array(...)))
        ...
        //    - (Jika `parse_range` gagal, error akan otomatis ter-propagate)
